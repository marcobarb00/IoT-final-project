...
uint16_t LightProtC$time_delays[1000][7];


bool LightProtC$route_req_sent[1000];
bool LightProtC$route_rep_sent[1000];


bool LightProtC$locked[1000];

static inline bool LightProtC$actual_send(uint16_t address, message_t *packet);
static bool LightProtC$generate_send(uint16_t address, message_t *packet, uint8_t type);
...
static inline message_t *LightProtC$Receive$receive(message_t *bufPtr, 
...
#line 63
{
  ...
  while (i < 8) {
      __nesc_hton_uint16(communication_channel.id.nxdata, i + 2);
...
#line 170
{
  if (err == SUCCESS) {
      if (TOS_NODE_ID == 1) {
          sim_log_debug(153U, "radio", "PANC radio start done\n");
          LightProtC$initialize_communication_channels();
        }
      else {
          sim_log_debug(154U, "radio", "Node %d: radio start done\n", TOS_NODE_ID);

          LightProtC$Timer3$startOneShot(2000);
        }
    }
  else {
      sim_log_error(155U, "radio", "Node %d: radio failed to start, retrying...\n", TOS_NODE_ID);
...
#line 188
{
  sim_log_debug(156U, "radio", "node %d: radio stopped\n", TOS_NODE_ID);
}
...
# 100 "/opt/tinyos-main-2.1.2/tos/interfaces/Send.nc"
inline static void /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$Send$sendDone(uint8_t arg_0xb74b0010, message_t * msg, error_t error){
#line 100
  switch (arg_0xb74b0010) {
#line 100
    case 0U:
#line 100
...
#line 100
    default:
#line 100
      /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$Send$default$sendDone(arg_0xb74b0010, msg, error);
#line 100
      break;
#line 100
    }
...
#line 53
# 80 "/opt/tinyos-main-2.1.2/tos/interfaces/AMSend.nc"
inline static error_t /*AMQueueP.AMQueueImplP*/AMQueueImplP$0$AMSend$send(am_id_t arg_0xb74b09f8, am_addr_t addr, message_t * msg, uint8_t len){
#line 80
  unsigned char __nesc_result;
#line 80

#line 80
  __nesc_result = TossimActiveMessageC$AMSend$send(arg_0xb74b09f8, addr, msg, len);
#line 80
...
#line 192
{
  ...
  sim_log_debug(157U, "timer", "node %d did not receive a CONNACK in time\n", TOS_NODE_ID);
...
#line 94
{
  ...
  sim_log_debug(149U, "radio_pack", "sending a subscribe message\n");

  subscribe_message = (msg_t *)LightProtC$Packet$getPayload(&LightProtC$packet_buf[sim_node()], sizeof(msg_t ));

  __nesc_hton_uint16(subscribe_message->type.nxdata, 2);
  __nesc_hton_uint16(subscribe_message->topic.nxdata, topic);


  LightProtC$generate_send(1, &LightProtC$packet_buf[sim_node()], 2);
}
...
#line 200
{
  ...
  sim_log_debug(158U, "timer", "node %d did not receive a SUBACK in time\n", TOS_NODE_ID);
...
# 83 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
inline static void /*HilTimerMilliC.VirtualizeTimerC*/VirtualizeTimerC$0$Timer$fired(uint8_t arg_0xb7330c70){
#line 83
  switch (arg_0xb7330c70) {
#line 83
    case 0U:
#line 83
...
#line 83
    default:
#line 83
      /*HilTimerMilliC.VirtualizeTimerC*/VirtualizeTimerC$0$Timer$default$fired(arg_0xb7330c70);
#line 83
      break;
#line 83
    }
...
#line 57
  {
    sim_log_debug(164U, "Init", "LEDS: initialized.\n");
...
#line 165
{
  sim_log_debug(152U, "boot", "Application booted.\n");
  LightProtC$AMControl$start();
}
...
#line 124
    {
      if (type == 1 && !LightProtC$route_req_sent[sim_node()]) {
          LightProtC$route_req_sent[sim_node()] = TRUE;
...
#line 130
        if (type == 2 && !LightProtC$route_rep_sent[sim_node()]) {
            LightProtC$route_rep_sent[sim_node()] = TRUE;
            LightProtC$Timer0$startOneShot(LightProtC$time_delays[sim_node()][TOS_NODE_ID - 1]);
            LightProtC$queued_packet[sim_node()] = *packet;
            LightProtC$queue_addr[sim_node()] = address;
          }
...
#line 227
{
  ...
  if (error == SUCCESS) {
      sim_log_debug(159U, "radio_send", "Packet sent...\n");
      sim_log_debug_clear(160U, "radio_send", " at time %s \n", sim_time_string());
      ...
      if (__nesc_ntoh_uint16(message->type.nxdata) == 1) {
          sim_log_debug(161U, "radio_send", "packet sent was of type CONNECT\n");
      ...
      if (__nesc_ntoh_uint16(message->type.nxdata) == 2) {
          sim_log_debug(162U, "radio_send", "packet sent was of type SUBSCRIBE\n");
    ...
    sim_log_debug(163U, "radio_send", "there was an error sending the packet\n");
    }
                               ...
                               uintptr_t* addr, size_t* size)
{
  ...
  if (!strcmp(varname, "LightProtC$route_req_sent"))
  {
    *addr = (uintptr_t)&LightProtC$route_req_sent[__nesc_mote];
    *size = sizeof(LightProtC$route_req_sent[__nesc_mote]);
    return 0;
  }
  if (!strcmp(varname, "LightProtC$route_rep_sent"))
  {
    *addr = (uintptr_t)&LightProtC$route_rep_sent[__nesc_mote];
    *size = sizeof(LightProtC$route_rep_sent[__nesc_mote]);
    return 0;
  }
...
static void __nesc_nido_initialise(int __nesc_mote)
{
  ...
  memcpy((void *)&LightProtC$time_delays[__nesc_mote], (void *)&(unsigned short [7]){ 61, 173, 267, 371, 479, 583, 689 }, sizeof LightProtC$time_delays[__nesc_mote]);
  LightProtC$route_req_sent[__nesc_mote] = FALSE;
  LightProtC$route_rep_sent[__nesc_mote] = FALSE;
